...
- *.annotations.md files: X (of Y total)
...
annotation file **in scope**. Do not skip files. Do not summarize early.

**In scope** means: all matching files if running a full analysis, or only files
dated after the cutoff date if running incrementally. In incremental mode, only
process files whose embedded YYYY-MM-DD date is strictly after the cutoff date.
