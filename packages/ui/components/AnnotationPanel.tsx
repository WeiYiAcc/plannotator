...
import { type Annotation, AnnotationType, type Block } from '../types';
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1.5">
        {sortedAnnotations.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-40 text-center px-4">
            ...
            <p className="text-xs text-muted-foreground">Select text to add annotations</p>
          </div>
        ) : (
          sortedAnnotations.map((ann) => (
            <AnnotationCard
              ...
              onEdit={
                onEdit ? (updates: Partial<Annotation>) => onEdit(ann.id, updates) : undefined
              }
            />
          ))
        )}
...
const AnnotationCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      {annotation.author && (
        <div
          className={`flex items-center gap-1.5 text-[10px] font-mono truncate mb-1.5 ${isCurrentUser(annotation.author) ? 'text-muted-foreground/60' : 'text-muted-foreground'}`}
        >
          <svg
