...
export const VaultBrowser: React.FC<VaultBrowserProps> = ({
  ...
  if (isLoading) {
    return <div className="p-3 text-[11px] text-muted-foreground">Loading vault...</div>;
  }
  ...
  if (error) {
    return (
      <div className="p-3 space-y-2">
        ...
        <button onClick={onFetchTree} className="text-[10px] text-primary hover:underline">
          Retry
        </button>
      </div>
    );
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="py-1 px-1">
        {tree.length === 0 ? (
          <div className="px-3 py-2 text-[11px] text-muted-foreground">No markdown files found</div>
        ) : (
          tree.map((node) => (
            <TreeNode
              key={node.path}
